...
(defmacro jde-usages-profile (&rest body) 
  ...
  `(let ((start-time (float-time (current-time))) 
     ...
     (setq end-time (float-time (current-time)))
     (message "jde-usages-profile : %d" (- end-time start-time))
    ret))
...
`jde-usages-get-current-signature' is used."
  ...
  (cl-labels ((jde-usages-dct-goto-caller (caller calling)
               ...
               (if (not ret)
                 ...
                 (let*
                     ((begin (point)) ;; goto-line places point at the beginning of the line
                      ...
                      (method-name-or-constructor (if is-constructor
                                                      (concat "new\\s *[[:word:]$.]*"
...
when calling this function."
  (let
      ((class-name (or class-name (read-from-minibuffer "Class name:" (jde-usages-get-current-class))))
