  ...
  (when (and
    ...
    (tree-buffer-defpopup-command jde-usages-ecb-show-type-hierarchy
      ...
      (lexical-let ((node node))
       (jde-usages-display-type-hierarchy (jde-usages-get-class-around-tag (tree-node-get-data node)) t)))

    (tree-buffer-defpopup-command jde-usages-ecb-show-subclasses
      "Show subclasses."
      (lexical-let ((node node))
       (jde-usages-display-subs-and-implementers t t (jde-usages-get-class-around-tag (tree-node-get-data node)) t)))

    (tree-buffer-defpopup-command jde-usages-ecb-show-superclasses
      "Show superclasses."
      (lexical-let ((node node))
       (jde-usages-display-superclasses (jde-usages-get-class-around-tag (tree-node-get-data node)) t)))

    (tree-buffer-defpopup-command jde-usages-ecb-show-callers
      "Show callers."
      (lexical-let ((node node))
       (jde-usages-display-call-tree nil (lambda () (jde-usages-tag-to-signature (tree-node-get-data node))))))
    
    (tree-buffer-defpopup-command jde-usages-ecb-show-overrides
      "Show overrides."
      (lexical-let ((node node))
       (jde-usages-display-subs-implementing-method (jde-usages-tag-to-signature (tree-node-get-data node)))))
