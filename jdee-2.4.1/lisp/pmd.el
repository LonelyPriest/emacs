...
(defcustom pmd-java-home "/usr/jdk/bin/java"
  "Java binary to run PMD with."
  :type 'file
  :group 'pmd )

(defcustom pmd-home "/usr/share/emacs/24.3/site-lisp/jdee-2.4.1/pmd-bin-5.0.4"
...
(defun pmd-java-ruleset-file()
  (let* ((path-separator (if (eq system-type 'windows-nt) ";" ":"))
         (path-slash     (if (eq system-type 'windows-nt) "\\" "/"))
         (pmd-lib     (concat pmd-home path-slash "lib" path-slash)))
    (concat pmd-lib path-slash "pmd_ruleset_java.xml")))
...
(defun pmd-file-or-dir (target)
  ...
  (let ((pmd-command
         (concat pmd-java-home " -Djava.ext.dirs=" (pmd-lib-dir) " net.sourceforge.pmd.PMD "
                           " -d " target " -f emacs " " -R " (pmd-java-ruleset-file))))
