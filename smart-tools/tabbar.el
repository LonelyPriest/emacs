...
(defcustom tabbar-speedkey-use nil
  "If set to t, show a speedkey (1~0) before tab, and switch
quickly using tabbar-speedkey-prefix + coresponding sppedkey."
  :group 'tabbar
  :type 'boolean)

(defcustom tabbar-speedkey-prefix (kbd "<f1>")
  "Prefix key for speedkey commands."
  :group 'tabbar
  :type 'string)

(defvar tabbar-speedkey-map nil
  "Keymap for tabbar-speedkey commands.")
...
(defun tabbar-list-insert-before (el pos li-sym)
  (let ((li-value (symbol-value li-sym)))
    (if (= pos 0)
	(set li-sym (cons el li-value))
      (setcdr (nthcdr (1- pos) li-value)
	      (cons el (nthcdr pos li-value))))))

(defun tabbar-move (tabset count)
  "Change the position of tab on the TABSET.
If COUNT is positive move the tab on right.  If COUNT is negative,
move the tab on left."
  (let* ((tabs (tabbar-tabs tabset))
	 (sel-tab (tabbar-selected-tab tabset))
	 (sel-index (position sel-tab tabs))
	 (mov-index (max 0 (min (+ sel-index count) (1- (length tabs))))))
    (unless (= sel-index mov-index)
      (set tabset (delq sel-tab tabs))
      (tabbar-list-insert-before sel-tab mov-index tabset)
      (tabbar-set-template tabset nil)
      (tabbar-select-tab sel-tab tabset)
      )))
...
(defun tabbar-adjust (tabset)
  ...
  (let* ((tabs (tabbar-tabs tabset))
    ...
    (when (< sel-index start-index)
      (setq start-index (if (= start-index (length tabs))
			    0
			  sel-index)))
    (when (> sel-index start-index)
...
(defun tabbar-debug ()
  ...
  (let* ((tabset (tabbar-current-tabset))
	 ...
	 view-width)
    (message "----")
    (message "Tabs[%d] = %s" (length tabs) tabs)
    (message "StartIndex = %d" start-index)
    (setq start-index (1- start-index))
    (message "New StartIndex = %d" start-index)
    (message "SelIndex = %d" sel-index)
    (when (< sel-index start-index)
      (setq start-index (if (= start-index (length tabs))
			    0
			  sel-index)))
    (when (> sel-index start-index)
      (setq view-width (tabbar-tabs-view-width tabs start-index sel-index))
      (message "ViewWidth = %d" view-width)
      (message "WindowWidth = %d" (window-width))
      (when (> view-width (window-width))
	;; increase start-index until selected tab is within window.
	(setq tabs (nthcdr start-index tabs))
	(while (and (< start-index sel-index)
	...
	(message "Adjust StartIndex = %d" start-index)
	(message "Adjust ViewWidth = %d" view-width)

	))
    (message "Final StartIndex = %d" start-index)))
...
(defun tabbar-scroll-left (event)
  "On mouse EVENT, scroll/move current tab set on left."
  (cond
   ((eq (event-basic-type event) 'mouse-1)
    (tabbar-scroll (tabbar-current-tabset) -1))
   ((eq (event-basic-type event) 'mouse-2)
    (tabbar-move (tabbar-current-tabset) -10000))
   ((eq (event-basic-type event) 'mouse-3)
...
(defun tabbar-scroll-left-help ()
  ...
  "mouse-1: scroll tabs left. mouse-2: move tab leftmost. mouse-3: move tab left")

(defun tabbar-scroll-right (event)
  "On mouse EVENT, scroll/move current tab set on right."
  (cond
   ((eq (event-basic-type event) 'mouse-1)
    (tabbar-scroll (tabbar-current-tabset) 1))
   ((eq (event-basic-type event) 'mouse-2)
...
(defun tabbar-scroll-right-help ()
  ...
  "mouse-1: scroll tabs right. mouse-2: move tab rightmost mouse-3: move tab right")
...
;;;###autoload
(defun tabbar-goto-group()
  "Prompt a group name and go."
  (interactive)
  (let* ((tablist (mapcar #'(lambda (tab)
			      (cons
			       (downcase (format "%s" (tabbar-tab-tabset tab)))
			       tab))
			  (tabbar-tabs (tabbar-get-tabsets-tabset))))
	 (read-group (completing-read "Tab group: " tablist nil t))
	 (tab (cdr (assoc read-group tablist))))
    (tabbar-click-on-tab tab)))
...
Must be a valid `header-line-format' template element."
  (if tabbar-buffer-group-mode
    ...
    (if tabbar-speedkey-use
	(format " %s%s " (tabbar-speedkey-key tab) (tabbar-tab-value tab))
...
(defun tabbar-sorted-tabs (tabset key)
  "Return the list of sorted tabs in TABSET, compared with KEY
function. KEY may be 'tabbar-tab-value or 'tabbar-tab-tabset."
  (sort (copy-seq (tabbar-tabs tabset))
	(lambda (a b)
	  (string< (funcall key a) (funcall key b)))))

(defun tabbar-tab-menu (tabset)
  "Return the menu list of tabs in TABSET."
  (mapcar
   (lambda (tab)
     (vector (tabbar-tab-value tab)
	     `(tabbar-click-on-tab ',tab)))
   (tabbar-sorted-tabs tabset 'tabbar-tab-value)))

(defun tabbar-tabgroup-menu ()
  "Return the menu list of tabsets."
  (mapcar
   (lambda (tab)
     (cons (format "%s" (tabbar-tab-tabset tab))
	   (cons (vector "<<group>>"
			 `(tabbar-click-on-tab ',tab))
		 (tabbar-tab-menu (tabbar-tab-tabset tab)))))
   (tabbar-sorted-tabs (tabbar-get-tabsets-tabset) 'tabbar-tab-tabset)))

(defun tabbar-tab-popup ()
  "Popup a tab context menu."
  (interactive)
  (popup-menu
   `("Tabbar"
     ("Quick Switch" .
      ,(tabbar-tab-menu (tabbar-current-tabset)))
     ("Switch To" .
      ,(tabbar-tabgroup-menu))
     )))

(defun tabbar-speedkey-key (tab)
  "Return the speedkey of tab."
  (let ((idx (position tab (tabbar-tabs (tabbar-tab-tabset tab)))))
    (cond
     ((and (>= idx 0)
	   (<= idx 8))
      (format "%d'" (1+ idx)))
...
(defun tabbar-speedkey-handler ()
  "Handler of tabbar speedkey."
  (interactive)
  (let (idx tab)
    (cond
     ((and (>= last-input-char ?1)
	   (<= last-input-char ?9))
      (setq idx (- last-input-char ?1))) ; '1'~'9' -> 0~8
     ((= last-input-char ?0)
      (setq idx 9))			; '0' -> 9
     ((and (>= last-input-char ?a)
	   (<= last-input-char ?z))
      (setq idx (+ 10 (- last-input-char ?a)))) ; 'a'~'z' -> 10~35
     (t
      (error "Unsupport speedkey")))
    (setq tab (elt (tabbar-tabs tabbar-current-tabset) idx))
    (when tab
      (tabbar-click-on-tab tab))))

(defun tabbar-speedkey-install ()
  "Install speedkey."
  (when (and tabbar-speedkey-use
	     tabbar-speedkey-prefix)
    (setq tabbar-speedkey-map (make-sparse-keymap))
    (dolist (k (append (number-sequence ?0 ?9) (number-sequence ?a ?z)))
      (define-key tabbar-speedkey-map (char-to-string k) 'tabbar-speedkey-handler))
    (global-set-key tabbar-speedkey-prefix tabbar-speedkey-map)
    ))

(defun tabbar-speedkey-uninstall ()
  "Uninstall speedkey."
  (and tabbar-speedkey-use
       tabbar-speedkey-prefix
       (global-unset-key tabbar-speedkey-prefix)))
